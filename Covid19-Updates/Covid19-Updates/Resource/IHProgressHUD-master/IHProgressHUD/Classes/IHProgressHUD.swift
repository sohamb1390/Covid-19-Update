    ...
    @objc private func positionHUD(_ notification: Notification? = nil) {
        ...
        if IHProgressHUD.isNotAppExtension {
            ...
            if #available(iOS 13.0, *) {
                orientation = self.window?.windowScene?.interfaceOrientation ?? .landscapeLeft
            } else {
        ...
        if let notificationData = notification {
            ...
            if #available(iOS 13.0, *) {
                statusBarFrame = self.window?.windowScene?.statusBarManager?.statusBarFrame ?? .zero
            } else {
...
//                self.frame = containerView!.frame
    ...
    private func updateHUDFrame() {
        ...
        if getStatusLabel().text != nil {
            ...
            labelRect = getStatusLabel().text?.boundingRect(with: constraintSize, options: [.usesFontLeading, .truncatesLastVisibleLine, .usesLineFragmentOrigin], attributes: [NSAttributedString.Key.font: getStatusLabel().font ?? UIFont.preferredFont(forTextStyle: .body)], context: nil) ?? CGRect.zero
